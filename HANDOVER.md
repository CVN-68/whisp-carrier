...
| 項目 | 状態（2026-09-03 時点） |
|------|----------------------|
| **ビルド済みだが未公開の exe** | **`1.0.0`。`998798a` のコード状態でビルドした 13,899,921 バイト。**[チェックリスト16項目は全部通っている](MEASUREMENTS.md#100のビルドと配布アーカイブ2026-09-03) |
| **未公開の配布アーカイブ** | `whisp-carrier-1.0.0.7z`、**1,055,059,470 バイト（1007 MiB・単一ファイル）。SHA-256 `52E84E784751EA1009F6701D0BA70CE2598C4D9161303DFFBD29E67C7F89AE9B`**。**アセット digest との照合は公開後。**[README の1行を直して1回固め直している](MEASUREMENTS.md#実況系の推奨が2箇所に残っていた100を再パックした2026-09-03)（旧 `6494C638…` は破棄） |
| アーカイブ内の docs | `README.md` 53,304 バイト・`THIRD-PARTY-NOTICES.md` 17,403 バイト。**どちらもソースとバイト一致** |
| **公開済みの exe** | **`0.9.2`。`923016c` のコード状態でビルドした 13,887,994 バイト。**[N](MEASUREMENTS.md#n-cuda_path-が同梱の-cublas-を隠していた2026-08-24-報告092-で修正)（`CUDA_PATH` が同梱 cuBLAS を隠す不具合）の修正が入っている。**RTX 4080 / 2070 で第三者が動作確認したのはこの版**（[記録](MEASUREMENTS.md#決着092-が-4080-と-2070-の両方で回避策なしに動いた2026-08-27-報告)） |
| 0.9.2 の配布アーカイブ | `whisp-carrier-0.9.2.7z`、1,055,081,794 バイト（1006 MiB）。SHA-256 `D6EEC1EC387821B9CDAB0726B5691863E714C65BB3DB050CCA6D96E51F51982F`（アセット digest と一致を確認済み） |
| main の HEAD | `998798a`。**1.0.0 はこのコード状態そのもの**（配布物と HEAD が一致しているのは版を上げた直後だけの状態） |
| タグ | **`v1.0.0` はまだ打っていない。打つのは main の HEAD**（exe のコード状態 `998798a` との差は記録の追記だけ）。**0.9.2 でローカルと remote がずれたのは exe のコード状態に打ったため**で、`gh release create` はタグが無ければ既定ブランチの HEAD に作る。過去分は remote で `v0.9.0` → `e165168` / `v0.9.1` → `0b73544` / `v0.9.2` → `2f39df8`。確認は `git fetch --tags` のあと `git ls-remote --tags origin` |
| 未 push | **9コミット。** `c89ea96` 〜 `4e2890e` |
| 過去の版 | `0.9.2` = `923016c`、`0.9.1` = `0b73544`（[J](MEASUREMENTS.md#j-同梱-ffmpeg-が使われていなかった2026-08-23) と [K](MEASUREMENTS.md#k-torch-を外して配布サイズを削る2026-08-23)）、`0.9.0` = `6ac58a5` |
...
自動化してもテスト自体が腐る。**pytest に載せるなら環境に依存しない純粋関数だけ**
