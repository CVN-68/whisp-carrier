...
| アーカイブ内の docs | `README.md` **53,304 バイト**・`THIRD-PARTY-NOTICES.md` 17,403 バイト。**固めた時点ではソースと一致していたが、公開後に経緯の節を直したので README にずれが出た**（ソースは 53,510 バイト、差 206）。**再パックしない** — [発動条件](#再パックの発動条件ドキュメントだけの変更では固め直さない)では「README の本文だけ」に当たり、**公開後は差し替えのほうが高い**（新 SHA-256・アセット差し替え・記録済みの digest が無効になる）。0.9.0 で 298 バイトのずれを許容したのと同じ判断 |
| ビルド済みだが未公開の exe | **無い** |
