...
> **現在は 0.9.2。この節の判断は変わっていない**（見出しはアンカーのため据え置き）。
> **1.0 の条件は「現行 exe が他人の環境で1回動くこと」で、まだ満たしていない。**
> 第三者環境での最初の試行（RTX 4080、2026-08-24）は
> [不具合の報告](MEASUREMENTS.md#n-cuda_path-が同梱の-cublas-を隠していた2026-08-24-報告092-で修正)で、
> 0.9.2 で直したが**修正版で動いたという報告はまだ無い。**
> **次に来るのは RTX 2070 での 0.9.2 の結果**で、それが通れば 1.0 に上げる。
...
> **送付済み（2026-08-23）。** 作者のブログのコメント欄で、exe 版の公開報告と
> 同じ書き込みに含めて訂正した。**この節は「何をどう訂正したか」の記録として残す。**
> **Amatsukaze 側のドキュメントに反映されたかは未確認**で、こちらから催促するものでもない。
...
| 項目 | 状態（2026-08-25 時点） |
|------|----------------------|
| **公開済みの exe** | **`0.9.2`（最新）。`923016c` のコード状態でビルドした 13,887,994 バイト。**[N](MEASUREMENTS.md#n-cuda_path-が同梱の-cublas-を隠していた2026-08-24-報告092-で修正)（`CUDA_PATH` が同梱 cuBLAS を隠す不具合）の修正が入っている |
| 配布アーカイブ | `whisp-carrier-0.9.2.7z`、1,055,081,794 バイト（1006 MiB・単一ファイル）。**SHA-256 `D6EEC1EC387821B9CDAB0726B5691863E714C65BB3DB050CCA6D96E51F51982F`**（リリースのアセット digest と一致を確認済み） |
| アーカイブ内の docs | `README.md` はソースと同一（50,216 バイト）。**ずれは無い** |
| ビルド済みだが未公開の exe | **無い** |
| main の HEAD | `c89ea96` 以降。**0.9.2 のコード状態（`923016c`）より先に進んでいるのが正常**（`2f39df8` は spec のコメント修正で exe に影響なし、`c89ea96` は記録の追記） |
| タグ | **`v0.9.2` は remote では `2f39df8`**（リリース作成時に GitHub 側が main から作った）。**ローカルは `923016c` を指したままなのでずれる。** `git tag -f v0.9.2 origin/main` で合わせる。確認は `git fetch --tags` のあと `git ls-remote --tags origin` |
| 過去の版 | `0.9.1` = `0b73544`（[J](MEASUREMENTS.md#j-同梱-ffmpeg-が使われていなかった2026-08-23) と [K](MEASUREMENTS.md#k-torch-を外して配布サイズを削る2026-08-23)）、`0.9.0` = `6ac58a5` |
...
| 6 | **`CUDA_PATH` を `cublas64_12.dll` の無いディレクトリに向ける** | exit 0・`[CUDA] ignoring CUDA_PATH` | **[N](MEASUREMENTS.md#n-cuda_path-が同梱の-cublas-を隠していた2026-08-24-報告092-で修正)** |
...
**`--ff_*` の精度への影響は1つも測っていない。** `eval/run.py` は `--ff_*` を
一切渡さないので、**このプロジェクトが報告している数値はすべてフィルター未経由。**
実運用も渡さない方針で決着した。**「実験的」はその状態の宣言で、
「掛けると悪くなる」という測定結果があるわけではない。**

**当初「`--ff_loudnorm` / `--ff_lowhighpass` / `--ff_vocal_extract` はセグメント数が
激減する」と書いていたが、その観測は順序バグがあった時代のもの**で、
**修正後に測り直していない。** 現在形で「激減する」と読まないこと。
バグの中身は「チャンネル選択が無音を作る（-91.0dB）」と「帯域通過が実質無効
（mean が 0.1dB しか動かない）」の2つで、**どちらも修正済み・計測基盤つき。**
詳細は[既知の問題 2](#2-音声フィルターが実用レベルに達していない)。
